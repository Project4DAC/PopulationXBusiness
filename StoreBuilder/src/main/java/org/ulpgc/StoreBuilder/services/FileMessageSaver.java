...
public class FileMessageSaver implements MessageSaver {
    ...
    public void saveMessage(String message) {
        try {
            ...
            System.out.println("Notification saved to DataLake: " + basePath + File.separator + notificationFilename);
            ...
            if (url != null && !url.isEmpty()) {
                try {
                    ...
                    if (content != null) {
                        // Create a better filename using date if available
                        String contentFilename = "borme_content_" +
                                (!date.isEmpty() ? date.replaceAll("-", "") : timestamp) + ".json";
                        saveToFile(content, contentFilename);
                        System.out.println("URL content saved to DataLake: " + basePath + File.separator + contentFilename);
                    }
                } catch (Exception e) {
